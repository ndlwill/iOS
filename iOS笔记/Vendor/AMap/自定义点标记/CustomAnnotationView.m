...
{
    NSLog(@"self.selected = %ld selected = %ld", [[NSNumber numberWithBool:self.selected] integerValue], [[NSNumber numberWithBool:selected] integerValue]);
...
- (BOOL)pointInside:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    NSLog(@"is inside = %ld", [[NSNumber numberWithBool:inside] integerValue]);
    
    // 点击callout相当于点击annoview
    // 点击callout也不移除callout
    if (!inside && self.selected)
    {
        NSLog(@"inside begin= %ld", [[NSNumber numberWithBool:inside] integerValue]);
        inside = [self.calloutView pointInside:[self convertPoint:point toView:self.calloutView] withEvent:event];
        NSLog(@"inside end= %ld", [[NSNumber numberWithBool:inside] integerValue]);
    }
