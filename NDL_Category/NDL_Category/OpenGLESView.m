...
/*
使用编译链接自定义的着色器（shader）。用简单的glsl语言来实现顶点、片元着色器，并让图形进行简单的变换。
思路：
  1.创建图层
  2.创建上下文
  3.清空缓存区
...
-(void)renderLayer
{
    ...
    //2.读取顶点着色程序、片元着色程序
    NSString *vertFile = [[NSBundle mainBundle]pathForResource:@"shader1" ofType:@"vsh"];
    NSString *fragFile = [[NSBundle mainBundle]pathForResource:@"shader1" ofType:@"fsh"];
...
-(void)setupLayer
{
    //1.创建特殊图层
    /*
     重写layerClass，将OpenGLESView返回的图层从CALayer替换成CAEAGLLayer
     */
    ...
    /*
     ...
     kEAGLDrawablePropertyColorFormat
         ...
         kEAGLColorFormatRGB565：16位RGB的颜色，// R:5 G:6 B:5 共占16位
